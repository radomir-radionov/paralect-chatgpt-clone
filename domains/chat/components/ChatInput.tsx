...
export function ChatInput({ roomId, author }: Props) {
  ...
  async function handleSubmit(e?: FormEvent) {
    ...
    if ((!text && !hasImages) || sendMessage.isPending || isOverLimit) return;
  ...
  const canSend = useMemo(() => {
    ...
    const hasImages = images.length > 0;
    return !sendMessage.isPending && !isOverLimit && (hasText || hasImages);
  }, [images.length, isOverLimit, message, sendMessage.isPending]);
